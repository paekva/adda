...
interface CartRepository : JpaRepository<Cart, Int> {
    ...
    @Modifying
    @Query(
        "INSERT INTO product_to_cart values (:cartId, :productId, 1) ON CONFLICT ON CONSTRAINT product_to_cart_pkey DO " +
                "UPDATE SET quantity = product_to_cart.quantity + EXCLUDED.quantity",
        nativeQuery = true
    )
